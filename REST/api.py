...
def noactcatrange(categoryName):
    if request.method == 'GET':
        ...
        if(categoryName, (int(startRange) >= 1), endRange):
            cnt = mycursor.execute("SELECT COUNT(*) FROM acts,category WHERE acts.catno1 = category.catno AND category.catname = %s", (categoryName, ))
            cnt = mycursor.fetchone()
            #cnt = jsonify(cnt)
            
            if((int(endRange) <= cnt[0])):
                cat6 = mycursor.execute("SELECT * FROM acts,category WHERE category.catno = acts.catno1 AND category.catname = %s ORDER BY times DESC", (categoryName, ))
...
def uploadact():
    if request.method == 'POST':
        ...
        time1 = time.strptime(data['timestamp'], '%Y-%m-%d:%H-%M-%S')
        ...
        cat6 = mycursor.execute("SELECT catno FROM category WHERE category.catname = %s", (catname, ))
        cat6 = mycursor.fetchone()
        if(cat6[0]):
            cat12 = mycursor.execute("SELECT actid FROM acts WHERE actid = %s", (actid, ))
            cat12 = mycursor.fetchone()
            if(not cat12[0]):
                cat3 = "INSERT INTO acts (actid,caption,uname,catno1,imgpath,times) VALUES(%s,%s,%s,%s,%s,%s)"
